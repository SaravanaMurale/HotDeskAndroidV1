    ...
    public void onBindViewHolder(@NonNull BookingListToEditViewHolder holder, int position) {
        ...
        switch (bookingsListToEdit.get(position).getUsageTypeId()){
            case 7:
                ...
                try {
                    if (teamDeskAvailabilities!=null){
                        for (int i=0;i<teamDeskAvailabilities.size();i++){
                            if (Utils.checkStringParms(bookingsListToEdit.get(position).getRequestedTeamDeskId())
                                    == Utils.checkStringParms(teamDeskAvailabilities.get(i).getTeamDeskId())){
